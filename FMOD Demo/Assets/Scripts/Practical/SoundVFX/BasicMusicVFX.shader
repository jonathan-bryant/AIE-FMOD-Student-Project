Shader "Unlit/BasicMusicVFX"
{
	Properties
	{
		...
		iResolutionX ("ResolutionX", Float) = 720.0
		iResolutionY ("ResolutionY", Float) = 512.0
	...
	SubShader
	{
		...
		Pass
		{
			...
			float audio_freq( in sampler2D channel, in float f) { return tex2Dlod( channel, float4(f, 0.25,0,0) ).x; }
			float audio_ampl( in sampler2D channel, in float t) { return tex2Dlod( channel, float4(t, 0.75,0,0) ).x; }
			...
			v2f vert (appdata v)
			{
				...
				float2 uv = v.uv / float2(iResolutionX, iResolutionY);
				float2 centered = 2.0 * v.uv - 1.0;
