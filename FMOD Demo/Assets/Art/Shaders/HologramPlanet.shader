...
/////// Diffuse:
                ...
                float3 node_7560 = ((1.0+sin((2.0*(_MainTex_var.b+(node_514.b*_FlashSpeed)))))*clamp(rim,0,1)*_HexColor.rgb);
                float3 hexes = node_7560;
                ...
                float4 glowColor = float4((lerp(node_5239,_SurfaceTex_var.rgb,node_5645)+lerp(node_5239,__RimColor.rgb,node_5645)),1.0);
                float3 node_5269 = mainColor;
                float colorAlpha = _SurfaceTex_var.a;
                float3 diffuseColor = (hexes+(glow*(glowColor+(node_5269*colorAlpha))));
