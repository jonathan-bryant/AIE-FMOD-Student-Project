...
Shader "Cameron/CustomMultiSkybox" 
{
	Properties 
	{
		...
		[Gamma] _Exposure("Exposure", Range(0, 8)) = 1.3

		// Cubemap Properties
		_CubeMapTint ("Tint Color", Color) = (.5, .5, .5, .5)
		_Rotation ("Rotation", Range(0, 360)) = 0
		[NoScaleOffset] _CubeMapTex ("Cubemap   (HDR)", Cube) = "grey" {}

		// Var to determine the blending between skyboxes
...
//				for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
		...
		// if y < 0 [eyeRay.y > 0] - sky
			...
			c1 = lerp(IN.skyColor, IN.groundColor, saturate(y));
		...
		#if defined(UNITY_COLORSPACE_GAMMA) && !SKYBOX_COLOR_IN_TARGET_COLOR_SPACE
			c1 = LINEAR_2_OUTPUT(c1);
		#endif

			half4 tex = texCUBE(_CubeMapTex, IN.texcoord);
			half3 c2 = DecodeHDR(tex, _CubeMapTex_HDR);
			c2 = c2 * _SkyTint.rgb * unity_ColorSpaceDouble.rgb;
			c2 *= _Exposure;

			half3 c3 = lerp(c1, c2, _Blend);
			return half4(c3, 1.0);
